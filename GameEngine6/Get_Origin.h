...
namespace N_Core_Control
{
	...
	protected:
		...
		static constexpr size_t Default_Level=999;

		static constexpr size_t Default_Max_HitNum=999;

		virtual constexpr Tower*& Get_Tower() = 0;

		Storge*& Get_Storge(E_Core_Type type);

		template<class T, size_t t_Level, size_t t_Max_HitNum>
		std::vector<T*>& Search();
	...
	public:
		...
		template<class T,size_t t_Level = Default_Level>
			requires derived_from<T, Base>&&
		same_as_not<T, Base>
		T* Get_Child();

		template<class T,size_t t_Level= Default_Level,size_t t_Max_HitNum =Default_Max_HitNum>
			requires derived_from<T, Base> &&
		same_as_not<T,Base>
	...
	template<class T, size_t t_Level>
		requires derived_from<T, Base>&&
	same_as_not<T, Base>
